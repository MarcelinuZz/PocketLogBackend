...
export const requestChangePasswordOTP = async (req, res) => {
    try {
        ...
        const [passwordRows] = await db.query('SELECT hashed_password FROM user_passwords WHERE user_id = ?', [userId]);
        if (passwordRows.length === 0) return res.status(404).json({ message: "Data password tidak ditemukan. Akun ini mungkin terdaftar via Google." });
        ...
        await db.query('DELETE FROM otp_verifications WHERE user_id = ? AND action_type = ?', [userId, 'change_password']);
        const otpCode = generateOTP();
        const expiresAt = new Date(Date.now() + 5 * 60 * 1000);
        await db.query('INSERT INTO otp_verifications (user_id, otp_code, action_type, expires_at) VALUES (?, ?, ?, ?)', [userId, otpCode, 'change_password', expiresAt]);
        ...
        res.status(200).json({ message: "Kode OTP telah dikirim ke email Anda.", challengeToken });
    } catch (err) {
        console.error("[Account Error - Change Password Request]:", err);
...
export const confirmChangePassword = async (req, res) => {
    try {
        ...
        if (!tokenResult.valid) return res.status(403).json({ message: tokenResult.message });
        ...
        if (otpRows.length === 0) return res.status(400).json({ message: "Kode OTP tidak valid atau sudah kedaluwarsa." });

        const hashedPassword = await bcrypt.hash(newPassword, 11);
        await db.query('UPDATE user_passwords SET hashed_password = ? WHERE user_id = ?', [hashedPassword, userId]);
        await db.query('DELETE FROM otp_verifications WHERE user_id = ? AND action_type = ?', [userId, 'change_password']);

        res.status(200).json({ message: "Password berhasil diubah." });
    } catch (err) {
        console.error("[Account Error - Change Password Confirm]:", err);
...
export const requestChangeEmailOTP = async (req, res) => {
    try {
        ...
        const [existingEmail] = await db.query('SELECT id FROM users WHERE email = ? AND id != ?', [newEmail, userId]);
        if (existingEmail.length > 0) return res.status(409).json({ message: "Email sudah digunakan oleh akun lain." });

        const [userRows] = await db.query('SELECT email FROM users WHERE id = ?', [userId]);
        if (userRows.length === 0) return res.status(404).json({ message: "User tidak ditemukan." });
        if (userRows[0].email === newEmail) return res.status(400).json({ message: "Email baru tidak boleh sama dengan email saat ini." });

        await db.query('DELETE FROM otp_verifications WHERE user_id = ? AND action_type = ?', [userId, 'change_email']);
        const otpCode = generateOTP();
        const expiresAt = new Date(Date.now() + 5 * 60 * 1000);
        await db.query('INSERT INTO otp_verifications (user_id, otp_code, action_type, target_value, expires_at) VALUES (?, ?, ?, ?, ?)', [userId, otpCode, 'change_email', newEmail, expiresAt]);
        ...
        res.status(200).json({ message: "Kode OTP telah dikirim ke email Anda saat ini.", challengeToken });
    } catch (err) {
        console.error("[Account Error - Change Email Request]:", err);
        res.status(500).json({ message: "Terjadi kesalahan saat mengirim OTP." });
    }
};
...
export const confirmChangeEmail = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Email berhasil diubah.", data: { email: newEmail } });
    } catch (err) {
        console.error("[Account Error - Change Email Confirm]:", err);
...
export const requestDeleteAccountOTP = async (req, res) => {
    try {
        ...
        await db.query('DELETE FROM otp_verifications WHERE user_id = ? AND action_type = ?', [userId, 'delete_account']);
        const otpCode = generateOTP();
        const expiresAt = new Date(Date.now() + 5 * 60 * 1000);
        await db.query('INSERT INTO otp_verifications (user_id, otp_code, action_type, expires_at) VALUES (?, ?, ?, ?)', [userId, otpCode, 'delete_account', expiresAt]);
        ...
        console.error("[Account Error - Delete Account Request]:", err);
        res.status(500).json({ message: "Terjadi kesalahan saat mengirim OTP." });
    }
};
...
export const confirmDeleteAccount = async (req, res) => {
    try {
        ...
        console.error("[Account Error - Delete Account Confirm]:", err);
